...
export class MikrotikService implements IDnsProvider {
  ...
  initialize(): void {
    this.baseUrl = this.configService.get<string>('MIKROTIK_BASEURL', {
      infer: true,
    })!;
    this.username = this.configService.get<string>('MIKROTIK_USERNAME', {
      infer: true,
    })!;
    ...
    const skipTLSVerify =
      this.configService.get<boolean>('MIKROTIK_SKIP_TLS_VERIFY', {
        infer: true,
      }) ?? false;
    this.defaultTTL =
      this.configService.get<number>('MIKROTIK_DEFAULT_TTL', { infer: true }) ??
      3600;
    this.entryIdentifier = this.configService.get<string>('ENTRY_IDENTIFIER', {
      infer: true,
    })!;
    this.authHeader = `Basic ${Buffer.from(`${this.username}:${password}`).toString('base64')}`;
    if (skipTLSVerify) {
      this.loggerService.warn(
        'MikrotikService: TLS verification disabled (MIKROTIK_SKIP_TLS_VERIFY=true)',
      );
  ...
  async createEntry(entry: DnsbaseEntry): Promise<void> {
    const body = this.factory.toCreateBody(
      entry,
  ...
  async updateEntry(
    ...
    await this.doRequest(
      'PATCH',
      `${this.baseUrl}/rest/ip/dns/static/${oldRecord.id}`,
      body,
    );
  }
  ...
  private mapRecord(raw: Record<string, unknown>): MikrotikProviderRecord {
    ...
    r.priority = raw['mx-preference']
      ? Number(raw['mx-preference'])
      : undefined;
    return r;
  }
